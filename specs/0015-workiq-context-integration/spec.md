...
1. The system shall expose a single backend API endpoint (`POST /api/workiq/search`) that proxies search queries to the WorkIQ MCP server, crafting a structured prompt requesting full details from the last 4 weeks, and returns the response as a categorized result
...
- **Single-phase query flow**: The search endpoint wraps the user's query in a structured prompt requesting full details from the last 4 weeks. WorkIQ returns a comprehensive response which is cleaned up (trailing AI suggestions stripped) and returned as a single result. There is no separate detail-fetching phase.
- **Backend API**: Two Express endpoints: `POST /api/workiq/search` accepts `{ query: string }` and returns `{ results: WorkIQResult[] }` (typically a single result containing the full WorkIQ response as the summary); `GET /api/workiq/status` reports availability. Search is proxied through a Next.js Route Handler (not rewrites) at `/api/backend/workiq/search/route.ts` to support long timeouts (90s).
